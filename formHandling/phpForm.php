<?php 
//checks to see if user selected major or not
function checkMajor(){
	if($_POST["studentMajor"] == ""){
		echo "You have not selected a major.<br/>";
	}
	else{
		echo "You have declared " . $_POST['studentMajor'] . " as your major.<br/>";
	}
}
//checks to see if program info checkbox was checked
function displayProgramInfo(){
	if(isset($_POST["contactInfo"]) == 1){
		echo"<li>" . "Choose to contact with information" . "</li>";
	}
}
//checks to see if program advisor checkbox was checkd
function displayAdvisor(){
	if(isset($_POST["contactAdvisor"]) == 2){
		echo "<li>" .  "Choose to contact by advisor" ."</li>";
	}
}
?>
<!DOCTYPE html>
...
?>
	<p>
		...
		<br/>
		Thank you for your interest in <?php echo $_POST["school_name"];?>
		<br/>
